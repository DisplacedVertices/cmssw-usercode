          ...
          if apply_correction:
            for tm in ["sim", "dat"] :
              ...
              if version == 'onnormdzulv30bmofftosspreselv10' :
                  batch = 'TrackMover_MixMoveVecEta_NoPreSelRelaxBSPNotwVetodR0p4JetBySameDecayJetHists' + version.capitalize() + '_%i%i_tau%06ium_M%02i_%s' % (nl, nb, tau, mg, batch_tag)
              else :
                  batch = 'TrackMover_MixEta_NoPreSelRelaxBSPNotwVetodR0p4JetByJetHists' + version.capitalize() + '_StopBbarBbar_%i%i_tau%06ium_M%02i_%s' % (nl, nb, tau, mg, batch_tag)
            ...
            if version == 'onnormdzulv30bmofftosspreselv10' :
                batch = 'TrackMover_MixMoveVecEta_NoPreSelRelaxBSPNotwVetodR0p4JetBySameDecayJetHists' + version.capitalize() + '_%i%i_%s' % (nl, nb, batch_tag)
            else :
