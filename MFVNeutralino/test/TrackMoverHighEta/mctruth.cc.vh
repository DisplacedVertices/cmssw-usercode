...
int main(int argc, char** argv) {
  ...
  nr.init_options("mfvMovedTreeMCTruth/t", "TrackMoverMCTruth_HighEta_HighdVV0p36_NoPreSelRelaxBSPVetodR0p4VetoMissLLPVetoTrkJetByMiniJetHistsOnnormdzUlv30lepmumv6", "trackmovermctruthonnormdzulv30lepmumv6", "all_signal = True")
    ;
    ...
    for (int ilsp = 0; ilsp < 2; ++ilsp) {
      ...
      if (dijet) {
        ...
        for (int j = 0; j < tks.n(); ++j) {
          ...
          if ( tks.pass_sel(j) && jp4.DeltaR(quark_p4_0) < 0.4 ){  
          ...
          if ( tks.pass_sel(j) && jp4.DeltaR(quark_p4_1) < 0.4 ){ 
            auto mt = std::find(minijettrk_idx.begin(), minijettrk_idx.end(), j);
            if (mt != minijettrk_idx.end()) continue;
