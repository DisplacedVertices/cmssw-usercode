...
#version = 'onnormdzulv30bmofftosspreselv10'
version = 'onnormdzulv30bmofftosspreselhighptv8'
...
for nl in 2,: # 2, 3:
    for nb in 0,: # 0, 2:
      for tau in [100, 300, 1000, 10000, 30000] :
        for mg in [ 400, 800,]: #[15, 40, 55,]: # 40, 55,] : #[15,40,55,]:
          if apply_correction:
            for tm in ["sim", "dat"] :
              w_fn_2d_move = ""
              if tm == "sim":
              ...
              if version == 'onnormdzulv30bmofftosspreselv10' :
                  batch = 'TrackMover_HighMoveVecEta_NoPreSelRelaxBSPNotwVetodR0p4JetBySameDecayJetHists' + version.capitalize() + '_%i%i_tau%06ium_M%02i_%s' % (nl, nb, tau, mg, batch_tag)
              else :
                  batch = 'TrackMover_HighEta_NoPreSelRelaxBSPNotwVetodR0p4JetByJetHists' + version.capitalize() + '_StopBbarBbar_%i%i_tau%06ium_M%02i_%s' % (nl, nb, tau, mg, batch_tag)
            ...
            if version == 'onnormdzulv30bmofftosspreselv10' :
                batch = 'TrackMover_HighMoveVecEta_NoPreSelRelaxBSPNotwVetodR0p4JetBySameDecayJetHists' + version.capitalize() + '_%i%i_%s' % (nl, nb, batch_tag)
            else :
