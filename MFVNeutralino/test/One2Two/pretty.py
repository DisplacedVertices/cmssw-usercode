...
g_thup.Draw('L')
g_thdn.Draw('L')
leg = ROOT.TLegend(0.324, 0.325, 0.608, 0.473)
leg.SetTextFont(42)
...
leg.Draw()
l1 = ROOT.TLine(706.185, 11849.8, 794.719, 11849.8)
l1.Draw()
l2 = ROOT.TLine(706.185, 12457.1, 794.719, 12457.1)
l2.Draw()
cms = write(61, 0.050, 0.10, 0.92, 'CMS')
pre = write(52, 0.040, 0.19, 0.92, 'Preliminary')
lum = write(42, 0.050, 0.60, 0.92, '17.6 fb^{-1} (8 TeV)')
c.SaveAs('/uscms/home/tucker/afshome/scan_lim_obs.pdf')
c.SaveAs('/uscms/home/tucker/afshome/scan_lim_obs.png')
