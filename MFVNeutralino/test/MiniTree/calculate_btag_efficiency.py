...
def btag_eff_per_event_from_btag_eff_per_jet(nvtx, event_flavor, bdisc):
  ...
  h_nbnl = f.Get('h_%dv_nljets_vs_nbjets' % nvtx)
  num = 0
  den = 0
  for nb in range(0, 1) if event_flavor == 'nobjets' else range(1, h_nbnl.GetNbinsX()):
    for nl in range(0, h_nbnl.GetNbinsY()):
      ...
      num += n_nbnl * (1 - (1-effb)**nb * (1-effl)**nl)
      den += n_nbnl
  return num/den
...
print '%10s%40s%40s%40s' % ('', 'per-jet', 'per-event from per-jet', 'per-event')
fmt = '%10s%20s%20s%20s%20s%20s%20s'
print fmt % ('', 'btag efficiency', 'fake rate', 'btag efficiency', 'fake rate', 'btag efficiency', 'fake rate')
for nvtx in [1, 2]:
  print '%d-track %d-vertex' % (ntk, nvtx)
  for bdisc in ['loose', 'medium', 'tight']:
    print fmt % (bdisc, '%.3f' % btag_eff_per_jet(nvtx, 'b', bdisc),                             '%.3f' % btag_eff_per_jet(nvtx, 'l', bdisc),
