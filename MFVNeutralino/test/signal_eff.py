...
ps = plot_saver(plot_dir('sigeff_%s_%s_April7_25' % (version, year)), size=(600,600), pdf=True, log=False)
...
for sample in samples: 
    ...
    den, _ = get_n('mfvEventHistosNoCuts')
    sample.y, sample.yl, sample.yh = clopper_pearson(num, den) # ignore integral != entries, just get central value right
    print '%26s: signal efficiency = %.3f (%.3f, %.3f)' % (sample.name, sample.y, sample.yl, sample.yh)

per = PerSignal('signal efficiency', y_range=(0.,max_eff)) 
if vh :
