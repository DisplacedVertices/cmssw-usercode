"""
Scan expected 95% CL UL vs first boundary position.
Scheme: 3-bin [0, x, 1.6, 4.0].
  Coarse scan: x = 0.2..1.4 in steps of 0.1 (existing)
  Fine scan:   x = 0.01..0.10 in steps of 0.01 (extended)

Usage:
  python3 boundary_scan.py gen     # generate datacards (LCG dev3 or cmsenv)
  python3 boundary_scan.py run     # run combine (cmsenv)
  python3 boundary_scan.py plot    # make plot (LCG dev3)
  python3 boundary_scan.py         # all three
"""
...
COARSE_VALS = [round(v * 0.1, 1) for v in range(2, 15)]  # 0.20..1.40 (existing)
...
def scheme_name(x):
    # fine range uses 'f' prefix with 3-digit millimeter representation
    if x < 0.15:
        return "scan_b1_f%03d" % round(x * 1000)
    return "scan_b1_%02d" % round(x * 10)

def scheme_bins(x):
    return [0., x, SECOND_BDY, UPPER]
...
def gen_all():
    ...
    for x in SCAN_VALS:
        ...
        # Check whether any signal/year datacards are missing
        missing = False
        for sig_id, ch in SIGNAL_POINTS:
            for yr in YEARS:
                p = os.path.join(DC_BASE, name, ch,
            ...
            if missing:
                break
        if not missing:
            print("SKIP %s (all datacards present)" % name)
            continue
        print("\n=== scan x=%.2f  %s ===" % (x, bins))
...
def plot(results):
    ...
    nom_x = 0.8
    # x-tick labels: show all fine points and every other coarse point
    ...
    for ax, sig_ids, title in [
        ...
        ax.axvline(nom_x, color="black", ls="--", lw=1.2, label="Nominal (0.8 cm)")
        ax.set_xlabel("First boundary (cm)", fontsize=12)
    ...
    fig.suptitle(r"Expected UL vs first boundary — scheme [0, $x$, 1.6, 4.0] cm",
                 fontsize=12)
    fig.tight_layout()

    for ext in ("pdf", "png"):
        out = os.path.join(HERE, "boundary_scan.%s" % ext)
