...
class do(object):
    ...
    def __init__(self,lo,hi,prnt=False):
        ...
        ne, be = ROOT.Double(), ROOT.Double()  #declare variable for event error and background error
        n = h.IntegralAndError(ibinlo, ibinhi, ne)  #total number of events and find the error value to assign to ne
        b = hbkg.IntegralAndError(ibinlo, ibinhi, be) #background events and find the error value to assign to be
        s = n - b  #total events minus background so this is signal events
        se = (ne**2 + be**2)**0.5  #signal error
        z = s / (b + be**2)**0.5
        p = s/n  #ratio of signal to total events
        if prnt:
