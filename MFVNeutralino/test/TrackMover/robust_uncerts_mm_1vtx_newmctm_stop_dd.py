...
# need to include LLP decays that are below TOC dVV flat curve but are affected by a geometric effect of a daughter quark's dxybs. We applied the cut before studying this. The overlapping region is universaly accepted to be < 300um (based on split-vertex merging)
if mass == '40':
  dvvcut = 0.25
elif mass == '15':
  dvvcut = 0.5
    ...
    for eta in ['Low','Mix','High']:
        ...
        for psd_method in psd_methods:
                ...
                if not reweight:
                    ...
                    sim_str = "/eos/uscms/store/user/pkotamni/TrackMover_B/TrackMover_%sEta_NoPreSelRelaxBSPNotwVetodR0p4JetByJetHistsOnnormdzulv30bmofftosspreselv8_20_tau%06ium_M%i_2DCorrection/background_btagpresel_%s.root" % (eta,int(ctau), int(mass), year)
                    dat_str = "/eos/uscms/store/user/pkotamni/TrackMover_B/TrackMover_%sEta_NoPreSelRelaxBSPNotwVetodR0p4JetByJetHistsOnnormdzulv30bmofftosspreselv8_20_tau%06ium_M%i_2DCorrection/BTagDispl%s.root" % (eta,int(ctau), int(mass), year)
                ...
                #print(ctau, str_ctau)
                signal  = ROOT.TFile('/uscms/home/pkotamni/nobackup/crabdirs/TrackMoverMCTruth_'+eta+'Eta_HighdVV_NoPreSelRelaxBSPVetodR0p4VetoMissLLPVetoTrkJetByMiniJetHistsOnnormdzUlv30bmpreselv6/mfv_stopdbardbar_tau'+ctau+'um_M'+mass+'_'+ year +'.root')
                ...
                signal_non  = ROOT.TFile('/uscms/home/pkotamni/nobackup/crabdirs/TrackMoverMCTruth_'+eta+'Eta_HighdVV_NoPreSelRelaxBSPVetodR0p4VetoMissLLPVetoTrkJetByMiniJetHistsOnnormdzUlv30bmpreselv6/mfv_stopdbardbar_tau'+ctau+'um_M'+mass+'_'+ year +'.root')  
                ...
                signal_vetopdvv  = ROOT.TFile('/uscms/home/pkotamni/nobackup/crabdirs/TrackMoverMCTruth_'+eta+'Eta_LowdVV_NoPreSelRelaxBSPVetodR0p4VetoMissLLPVetoTrkJetByMiniJetHistsOnnormdzUlv30bmpreselv6/mfv_stopdbardbar_tau'+ctau+'um_M'+mass+'_'+ year +'.root')  
...
ps = plot_saver(plot_dir('TM_Results_June5_2026_stop_dd_m%s' % mass), size=(700,600), pdf=True, log=False)
canvas = ps.c
