...
# we put dummy numbers in for 100um, but unless we have the input files and could slot these in. It would be better to drop them, but the cosmetics are bad then / the 1.0 label is chopped off
list_eff = [2000]
list_eff_2 = [2000]
list_ctau = [ 0.1, 1.0, 10.0,]
list_ctau = np.log10(list_ctau)
list_err = [0.0]
list_err_2 = [0.0]
list_relerr = [2000]
list_relerr_2 = [2000]
list_data_MC_ratio = [2000]
list_data_MC_ratio_2 = [2000]
list_new_err = [0.0]
list_new_err_2 = [0.0]
list_new_relerr = [2000]
list_new_relerr_2 = [2000]
  ...
  for ctau in ctaus:
    ...
    for eta in ['Low','Mix','High']:
        ...
        for psd_method in psd_methods:
                ...
                print(ctau, str_ctau)
                signal  = ROOT.TFile('/uscms/home/pkotamni/nobackup/crabdirs/TrackMoverMCTruth_'+eta+'Eta_HighdVV_NoPreSelRelaxBSPVetodR0p4VetoMissLLPVetoTrkJetByMiniJetHistsOnnormdzUlv30bmpreselv6/ggHToSSTodddd_tau'+str_ctau+'_M'+ mass +'_'+ year +'.root')
                ...
                signal_non  = ROOT.TFile('/uscms/home/pkotamni/nobackup/crabdirs/TrackMoverMCTruth_'+eta+'Eta_HighdVV_NoPreSelRelaxBSPVetodR0p4VetoMissLLPVetoTrkJetByMiniJetHistsOnnormdzUlv30bmpreselv6/ggHToSSTodddd_tau'+str_ctau+'_M'+ mass +'_'+ year +'.root')
                ...
                signal_vetopdvv  = ROOT.TFile('/uscms/home/pkotamni/nobackup/crabdirs/TrackMoverMCTruth_'+eta+'Eta_LowdVV_NoPreSelRelaxBSPVetodR0p4VetoMissLLPVetoTrkJetByMiniJetHistsOnnormdzUlv30bmpreselv6/ggHToSSTodddd_tau'+str_ctau+'_M'+ mass +'_'+ year +'.root')
...
ps = plot_saver(plot_dir('TM_Results_June5_2026_ggH_m%s' % mass), size=(700,600), pdf=True, log=False)
canvas = ps.c
...
ROOT.gPad.Update()
canvas.Update()
ps.save('gghss_'+str(mass)+'GeV_mmCtau_eff')
...
ROOT.gPad.Update()
canvas2.Update()
ps.save('gghss_'+str(mass)+'GeV_mmCtau_relerr')
...
ROOT.gPad.Update()
canvas.Update()
ps.save('gghss_'+str(mass)+'GeV_mmCtau_SF')
