...
int main(int argc, char** argv) {
  ...
  nr.init_options("mfvMovedTreeMCTruth/t", "TrackMoverMCTruth_HighdVV_NoPreSelRelaxBSPVetodR0p4VetoMissLLPVetoTrkJetByMiniJetHistsOnnormdzUlv30lepmumv6", "trackmovermctruthonnormdzulv30lepmumv6", "all_signal = True")
  ...
  for (numdens& nd : nds) {
    ...
    nd.book(k_vtxunc_2vtx, "vtxunc_2vtx", ";dist3d(move vector, vtx) cm; arb. units", 200, 0, 0.2);
    ...
    nd.book(k_sumdbv_2vtx, "sumdbv_2vtx", "; #Sigma d_{BV};events", 200, 0, 1.0);
    ...
    nd.book(k_vtx5tkunc_2vtx, "vtx5tkunc_2vtx", ";3D-distance of a vtx to an LLP/5tk-vtx cm. ;events", 100, 0, 0.04);
  ...
  auto fcn = [&]() {
    ...
    for (int ilsp = 0; ilsp < 2; ++ilsp) {
      ...
      if (dijet) {
        ...
        //if (lspdecay1.Mag() < 0.1 || lspdecay0.Mag() < 0.1 || lspdecay0.Mag()/lspdecay1.Mag() < 0.9 || lspdecay0.Mag()/lspdecay1.Mag() > 1.1)
        //  continue;

        //if (movedist3 < 0.5)
        //  continue;

        //if (lsp_p4.Beta()*lsp_p4.Gamma() > 10.0)
        //  continue;
      ...
      int n_pass_all = 0;
      double  dist2move = 100;
      jmt::MinValue dist2min(100);
      std::vector<int> first_vtx_to_pass(num_numdens, -1);
      double vtx_bs2derr = -9.9, vtx_eta = -9.9, vtx_z = -999.9, vtx_dbv = -999.9, vtx_3dbv = -999.9, vtx_ntk = -9, vtx_chi2 = -9.9; // JMTBAD ??? these end up with what???
      ...
      for (size_t i = 0; i < nvtx; ++i) {
        ...
        //if (!(dist2move > 0.0020 && dist2move < 0.0080)) //FIXME
        //  continue;
      ...
      //if (n_closeseedtks < 2 || n_closeseedtks > 4)
      //  continue;

      //if (n_closeseedtks < 6 || n_closeseedtks > 8)
      //  continue;

      //if (!((dist2move > 0.0030 && dist2move < 0.0050) || dist2move > 99)) //FIXME
      //  continue;
    ...
    double  dist2move_0 = 100;
    double  dist2move_1 = 100;
    ...
    dist2move_0 = dist2min_0.v();
    dist2move_1 = dist2min_1.v();
    ...
    for (int in = 0; in < num_numdens; ++in) {
      ...
      if (count_llp != 2) //FIXME
        continue;
      if (vtx0_ntk == 5 && vtx1_ntk == 5) {
        nd.den(k_vtx5tkunc_2vtx, dist2move_0);
        nd.den(k_vtx5tkunc_2vtx, dist2move_1); 
      }
      nd.den(k_vtxunc_2vtx, dist2move_0);
      nd.den(k_vtxunc_2vtx, dist2move_1);
      nd.den(k_sumdbv_2vtx, sumdbv);
    ...
    for (int in = 0; in < num_numdens; ++in) {
      ...
      numdens& nd = nds[in];
      if (vtx0_ntk == 5 && vtx1_ntk == 5) {
        nd.num(k_vtx5tkunc_2vtx, dist2move_0);
        nd.num(k_vtx5tkunc_2vtx, dist2move_1); 
      }
      nd.num(k_vtxunc_2vtx, dist2move_0);
      nd.num(k_vtxunc_2vtx, dist2move_1);
      nd.num(k_sumdbv_2vtx, sumdbv);
