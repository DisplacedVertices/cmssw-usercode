...
namespace mfv {
  ...
  void MovedTracksNtuple::clear() {
    ...
    gen_partons_in_acc = npu = npv = nlep = nseltracks = nalljets = npreseljets = npreselbjets = nlightjets = 0;
    pass_clean = false;
    pvntracks = ntracks = 0;
    jets_pt.clear();
    jets_eta.clear();
  ...
  void MovedTracksNtuple::write_to_tree(TTree* tree) {
    ...
    tree->Branch("gen_lsp_decay", gen_lsp_decay, "gen_lsp_decay[2*3]/F");
    ...
    tree->Branch("pass_clean", &pass_clean, "pass_clean/O");
    tree->Branch("npu", &npu);
    tree->Branch("npv", &npv);
    tree->Branch("pvx", &pvx);
    tree->Branch("pvy", &pvy);
  ...
  void MovedTracksNtuple::read_from_tree(TTree* tree) {
    ...
    tree->SetBranchAddress("pass_clean", &pass_clean);
