...
class MFVVertexHistos : public edm::EDAnalyzer {
 ...
 private:
  ...
  TH1F* h_sv_track_dxy_rescale_err[sv_num_indices];
  TH1F* h_sv_track_dz_err[sv_num_indices];
  TH1F* h_sv_track_rescale_nsigmadxy[sv_num_indices];
    ...
    do_scatterplots(cfg.getParameter<bool>("do_scatterplots"))
{
  ...
  for (int j = 0; j < sv_num_indices; ++j) {
    ...
    h_sv_track_dxy_rescale_err[j] = fs->make<TH1F>(TString::Format("h_sv_%s_track_dxy_rescale_err", exc), TString::Format(";%s SV tracks rescaled #sigma(dxy) (cm);arb. units", exc), 100, 0, 0.1);
    ...
    h_sv_track_rescale_nsigmadxy[j] = fs->make<TH1F>(TString::Format("h_sv_%s_track_rescale_nsigmadxy", exc), TString::Format(";%s SV tracks rescaled n#sigma(dxy);arb. units", exc), 400, 0, 40);
...
void MFVVertexHistos::analyze(const edm::Event& event, const edm::EventSetup&) {
  ...
  for (int isv = 0; isv < nsv; ++isv) {
    ...
    for (int i = 0; i < ntracks; ++i) {
      ...
      fill(h_sv_track_dxy_rescale_err, isv, aux.track_dxy_rescale_err(i), w);
      fill(h_sv_track_dz_err, isv, aux.track_dz_err(i), w);
      fill(h_sv_track_rescale_nsigmadxy, isv, aux.track_dxy_rescale_nsigma(i), w);
