...
class ABCDHistos : public edm::EDAnalyzer {
 ...
 private:
  ...
  TH1F* h_nsv;
  TH2F* h_sv_best0_bs2ddist_ntracks;
    ...
    which_mom(cfg.getParameter<int>("which_mom"))
{
  ...
  h_bs2ddist_ntracks = fs->make<TH2F>("h_bs2ddist_ntracks", ";ntracks;bs2ddist", 40, 0, 40, 100, 0, 1);
...
void ABCDHistos::analyze(const edm::Event& event, const edm::EventSetup&) {
  ...
  for (int isv = 0; isv < nsv; ++isv) {
    ...
    h_bs2ddist_ntracks->Fill(vtx.ntracks(), vtx.bs2ddist, w);
    h_bs2derr_ntracks->Fill(vtx.ntracks(), vtx.bs2derr, w);
    h_bs2dsig_ntracks->Fill(vtx.ntracks(), vtx.bs2dsig(), w);
  ...
  if (nsv >= 2) {
    ...
    const int ntracks01 = v0.ntracks() + v1.ntracks();
    const double maxtrackpt01 = v0.maxtrackpt() + v1.maxtrackpt();
    const double mass01 = v0.mass[which_mom] + v1.mass[which_mom];
    const double bs2ddist01 = v0.bs2ddist + v1.bs2ddist;
    const double pv2ddist01 = v0.pv2ddist + v1.pv2ddist;
    const double pv3ddist01 = v0.pv3ddist + v1.pv3ddist;
    const double pv3dctau01 = v0.pv3dctau() + v1.pv3dctau();
    const double svdist2d = mag(v0.x - v1.x, v0.y - v1.y);
    ...
    h_bs2ddist01_mass01->Fill(mass01, bs2ddist01, w);
    ...
    h_pv2ddist01_mass01->Fill(mass01, pv2ddist01, w);
    h_pv2ddist0_mass0->Fill(v0.mass[which_mom], v0.pv2ddist, w);

    h_pv3ddist01_mass01->Fill(mass01, pv3ddist01, w);
    h_pv3ddist0_mass0->Fill(v0.mass[which_mom], v0.pv3ddist, w);

    h_pv3dctau01_mass01->Fill(mass01, pv3dctau01, w);
    h_pv3dctau0_mass0->Fill(v0.mass[which_mom], v0.pv3dctau(), w);

    h_svdist2d_mass01->Fill(mass01, svdist2d, w);
    h_svdist3d_mass01->Fill(mass01, svdist3d, w);

    const double phibs0 = atan2(v0.y - mevent->bsy, v0.x - mevent->bsx);
    const double phibs1 = atan2(v1.y - mevent->bsy, v1.x - mevent->bsx);
    const double phipv0 = atan2(v0.y - mevent->pvy, v0.x - mevent->pvx);
    const double phipv1 = atan2(v1.y - mevent->pvy, v1.x - mevent->pvx);
    const double deltaphibs01 = reco::deltaPhi(phibs0, phibs1);
    const double deltaphipv01 = reco::deltaPhi(phipv0, phipv1);
    h_absdeltaphibs01->Fill(fabs(deltaphibs01), w);
    h_absdeltaphipv01->Fill(fabs(deltaphipv01), w);

    TLorentzVector x0(v0.x - pvx, v0.y - pvy, v0.z - pvz, v0.pv3ddist / v0.p4().Beta());
    TLorentzVector x1(v1.x - pvx, v1.y - pvy, v1.z - pvz, v1.pv3ddist / v1.p4().Beta());
    ...
    const TVector3 betacmz = TVector3(0, 0, -(p0.Pz() + p1.Pz()) / (p0.E() + p1.E()));
    x0.Boost(betacmz);
    x1.Boost(betacmz);
    p0.Boost(betacmz);
    p1.Boost(betacmz);
    ...
    const double svdist2dcmz = mag(x0.X() - x1.X(), x0.Y() - x1.Y());
    const double svdist3dcmz = mag(x0.X() - x1.X(), x0.Y() - x1.Y(), x0.Z() - x1.Z());
    h_svdist2dcmz_mass01->Fill(mass01, svdist2dcmz, w);
    h_svdist3dcmz_mass01->Fill(mass01, svdist3dcmz, w);

    const double svctau2dcmz = 2 * svdist2dcmz / (p0.Beta()*p0.Gamma() + p1.Beta()*p1.Gamma());
    const double svctau3dcmz = 2 * svdist3dcmz / (p0.Beta()*p0.Gamma() + p1.Beta()*p1.Gamma());
    h_svctau2dcmz_mass01->Fill(mass01, svctau2dcmz, w);
    h_svctau3dcmz_mass01->Fill(mass01, svctau3dcmz, w);

    //ntracks
    h_ntracks0_ntracks1->Fill(v1.ntracks(), v0.ntracks(), w);

    h_bs2ddist01_ntracks01->Fill(ntracks01, bs2ddist01, w);
    h_bs2ddist0_ntracks01->Fill(ntracks01, v0.bs2ddist, w);
    ...
    h_pv2ddist01_ntracks01->Fill(ntracks01, pv2ddist01, w);
    h_pv2ddist0_ntracks01->Fill(ntracks01, v0.pv2ddist, w);
    h_pv2ddist0_ntracks0->Fill(v0.ntracks(), v0.pv2ddist, w);

    h_pv3ddist01_ntracks01->Fill(ntracks01, pv3ddist01, w);
    h_pv3ddist0_ntracks01->Fill(ntracks01, v0.pv3ddist, w);
    h_pv3ddist0_ntracks0->Fill(v0.ntracks(), v0.pv3ddist, w);

    h_pv3dctau01_ntracks01->Fill(ntracks01, pv3dctau01, w);
    h_pv3dctau0_ntracks01->Fill(ntracks01, v0.pv3dctau(), w);
    h_pv3dctau0_ntracks0->Fill(v0.ntracks(), v0.pv3dctau(), w);

    h_svdist2d_ntracks01->Fill(ntracks01, svdist2d, w);
    h_svdist3d_ntracks01->Fill(ntracks01, svdist3d, w);

    h_svdist2dcmz_ntracks01->Fill(ntracks01, svdist2dcmz, w);
    h_svdist3dcmz_ntracks01->Fill(ntracks01, svdist3dcmz, w);
    ...
    h_pv2ddist01_njets->Fill(mevent->njets(), pv2ddist01, w);
    h_pv3ddist01_njets->Fill(mevent->njets(), pv3ddist01, w);
    h_pv3dctau01_njets->Fill(mevent->njets(), pv3dctau01, w);
    ...
    //other variables
    h_svctau3dcmz_maxtrackpt01->Fill(maxtrackpt01, svctau3dcmz, w);
    ...
    const double msptm0 = sqrt(v0.mass[which_mom] * v0.mass[which_mom] + v0.pt[which_mom] * v0.pt[which_mom]) + fabs(v0.pt[which_mom]);
