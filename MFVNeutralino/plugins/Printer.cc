...
void MFVPrinter::analyze(const edm::Event& event, const edm::EventSetup& setup) {
  ...
  if (print_event) {
    ...
    for (size_t i = 0; i < nl; ++i)
      printf("id: 0x%08x   pt: %11.3g   eta: %11.3g   phi: %11.3g   dxy: %11.3g   dz: %11.3g   iso: %11.3g\n", mevent->lep_id[i], mevent->lep_pt[i], mevent->lep_eta[i], mevent->lep_phi[i], mevent->lep_dxy[i], mevent->lep_dz[i], mevent->lep_iso[i]);
