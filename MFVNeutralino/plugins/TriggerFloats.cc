...
void MFVTriggerFloats::produce(edm::Event& event, const edm::EventSetup& setup) {
    ...
    if( year == 20161 || year == 20162 ) {
      metfilternames = {"Flag_goodVertices", "Flag_globalSuperTightHalo2016Filter", "Flag_HBHENoiseFilter", "Flag_HBHENoiseIsoFilter", "Flag_EcalDeadCellTriggerPrimitiveFilter", "Flag_BadPFMuonFilter", "Flag_BadPFMuonDzFilter", "Flag_eeBadScFilter", "Flag_hfNoisyHitsFilter"};
    }
    ...
    for (pat::TriggerObjectStandAlone obj : *trigger_objects) {
        ...
        if (obj.collection() == "hltFullCandsForHighMultTrigger::HLT") {
            floats->hlttracks.push_back(p4(obj.pt(), obj.eta(), obj.phi(), obj.energy()));
        }
            ...
            // Save all HLT PF btags
            if(obj.collection() == "hltPFJetForBtag::HLT") {
                bool save_this_jet = false;
                for (auto label : obj.filterLabels()) {
                    if (label == "hltBTagPFCSVp070Triple") { save_this_jet = true; }    // 2017 bjets
                    if (label == "hltBTagPFDeepCSV4p5Triple") { save_this_jet = true; } // 2018 bjets
                }
                if (save_this_jet) {
            ...
            // Save HLT Calo BJets
            else if (obj.collection() == "hltSelector6CentralJetsL1FastJet::HLT") {
                bool save_this_jet = false;
                for (auto label : obj.filterLabels()) {
                    if (label == "hltBTagCalo80x6CSVp0p92DoubleWithMatching") { save_this_jet = true; } // 2017 bjets
                    if (label == "hltBTagCaloDeepCSV0p71Double6Jets80")       { save_this_jet = true; } // 2018 bjets
                    if (label == "hltBTagCaloCSVp014DoubleWithMatching")      { save_this_jet = true; } // 2016 high score bjets
                }
                if (save_this_jet) {
                    floats->hltcalobjets.push_back(p4(obj.pt(), obj.eta(), obj.phi(), obj.energy()));
                }
            }

            // Save low-score HLT Calo BJets (for two of the 2016 triggers)
            else if (obj.collection() == "hltSelector8CentralJetsL1FastJet::HLT") {
                bool save_this_jet = false;
                for (auto label : obj.filterLabels()) {
                    if (label == "hltBTagCaloCSVp087Triple")      { save_this_jet = true; } // 2016 low score bjets
                }
                if (save_this_jet) {
                    floats->hltcalobjets_low.push_back(p4(obj.pt(), obj.eta(), obj.phi(), obj.energy()));
                }
            }
            ...
            // Mid-momentum calo hets with not very many prompt tracks
    ...
    //std::cout << "Finishing TriggerFloats.cc" << std::endl;
