...
class MFVSecondaryVertexProducer : public edm::EDProducer {
    public:
	explicit MFVSecondaryVertexProducer(const edm::ParameterSet &params);
	~MFVSecondaryVertexProducer();
...
MFVSecondaryVertexProducer::ConstraintType
MFVSecondaryVertexProducer::getConstraintType(const std::string &name)
{
	if (name == "None")
		...
		throw cms::Exception("InvalidArgument")
			<< "MFVSecondaryVertexProducer: ``constraint'' parameter "
...
getGhostTrackFitType(const std::string &name)
{
	if (name == "AlwaysWithGhostTrack")
		...
		throw cms::Exception("InvalidArgument")
			<< "MFVSecondaryVertexProducer: ``fitType'' "
			   "parameter value \"" << name << "\" for "
			   "GhostTrackVertexFinder settings not "
			   "understood." << std::endl;
}

MFVSecondaryVertexProducer::MFVSecondaryVertexProducer(
					const edm::ParameterSet &params) :
	trackIPTagInfoLabel(params.getParameter<edm::InputTag>("trackIPTagInfos")),                  // "impactParameterTagInfos"
	sortCriterium(TrackSorting::getCriterium(params.getParameter<std::string>("trackSort"))),    // 'sip3dSig' -> TrackIPTagInfo::IP3DSig
	trackSelector(params.getParameter<edm::ParameterSet>("trackSelection")),                     // cms.PSet(totalHitsMin = cms.uint32(8),
					                                                             ...
					                                                             //          normChi2Max = cms.double(99999.9)
                                                                                                     //          )
	constraint(getConstraintType(params.getParameter<std::string>("constraint"))),               // 'BeamSpot' -> CONSTRAINT_BEAMSPOT
	constraintScaling(1.0),
	vtxRecoPSet(params.getParameter<edm::ParameterSet>("vertexReco")),                           // cms.PSet(seccut = cms.double(6.0),
					                                                             ...
					                                                             //          finder = cms.string('avr')
                                                                                                     //          )
	useGhostTrack(vtxRecoPSet.getParameter<std::string>("finder") == "gtvr"),                    // 'avr' -> false
	withPVError(params.getParameter<bool>("usePVError")),                                        // True
	minTrackWeight(params.getParameter<double>("minimumTrackWeight")),                           // 0.5
	vertexFilter(params.getParameter<edm::ParameterSet>("vertexCuts")),                          // cms.PSet(distSig3dMax = cms.double(99999.9),
					                                                             ...
					                                                             //          distSig3dMin = cms.double(-99999.9)
                                                                                                     //          )
	vertexSorting(params.getParameter<edm::ParameterSet>("vertexSelection"))                     // cms.PSet( sortCriterium = cms.string('dist3dError') )
{
	...
	if (constraint == CONSTRAINT_PV_BEAMSPOT_SIZE ||
        ...
        useExternalSV = false;
        if(params.existsAs<bool>("useExternalSV")) useExternalSV = params.getParameter<bool> ("useExternalSV");    // True
        if(useExternalSV) {
	  extSVCollection  = params.getParameter<edm::InputTag>("extSVCollection");  // "inclusiveMergedVertices"
	  extSVDeltaRToJet = params.getParameter<double>("extSVDeltaRToJet");        // 0.3
        }
...
void MFVSecondaryVertexProducer::produce(edm::Event &event,
                                      const edm::EventSetup &es)
{
	...
	event.getByLabel(trackIPTagInfoLabel, trackIPTagInfos); // "impactParameterTagInfos"

        // External Sec Vertex collection (e.g. for IVF usage)
        edm::Handle<reco::VertexCollection> extSecVertex;          
        if(useExternalSV) event.getByLabel(extSVCollection,extSecVertex); // useExternalSV=True, "inclusiveMergedVertices"
                                                             

	edm::Handle<BeamSpot> beamSpot;
	unsigned int bsCovSrc[7] = { 0, };
	double sigmaZ = 0.0, beamWidth = 0.0;
	switch(constraint) {  // "BeamSpot"
	    case CONSTRAINT_PV_BEAMSPOT_SIZE:
	      event.getByLabel(beamSpotTag,beamSpot); // "offlineBeamSpot"
...
DEFINE_FWK_MODULE(MFVSecondaryVertexProducer);
