...
void MFVSignalMatch::analyze(const edm::Event& event, const edm::EventSetup&) {
  ...
  for (int i = 0; i < 2; ++i) {
    const double genrho = mag(mevent->gen_lsp_decay[i*3+0] - mevent->gen_pv[0],
                              mevent->gen_lsp_decay[i*3+1] - mevent->gen_pv[1]);
    h_genrho->Fill(genrho);
    if (match_ivtx[i] == -1) {
      h_genrho_notmatched->Fill(genrho);
      if (nvtx >= 2)
        h_genrho_notmatched_nge2->Fill(genrho);
    }
  }
}
