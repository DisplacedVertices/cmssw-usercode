    ...
    gen_jet_src(cfg.getParameter<edm::InputTag>("gen_jet_src"))
{
  ...
  h_gen_dbv_matched = fs->make<TH1F>("h_gen_dbv", ";generated d_{BV};generated LSPs with a reconstructed vertex within 120 #mum", 100, 0, 0.5);
...
*/
  ...
  for (const MFVVertexAux& vtx : *vertices) {
    ...
    h_dx->Fill((vtx.x - x0) - v[ilsp][0]);
    h_dy->Fill((vtx.y - y0) - v[ilsp][1]);
    h_dz->Fill((vtx.z - z0) - v[ilsp][2]);
    h_dist2d->Fill(mag(v[ilsp][0] - (vtx.x - x0),
                       v[ilsp][1] - (vtx.y - y0)));
    h_dist3d->Fill(mag(v[ilsp][0] - (vtx.x - x0),
                       v[ilsp][1] - (vtx.y - y0),
                       v[ilsp][2] - (vtx.z - z0)));
    ...
    h_pull_dx->Fill(((vtx.x - x0) - v[ilsp][0]) / sqrt(vtx.cxx));
    h_pull_dy->Fill(((vtx.y - y0) - v[ilsp][1]) / sqrt(vtx.cyy));
    h_pull_dz->Fill(((vtx.z - z0) - v[ilsp][2]) / sqrt(vtx.czz));
    h_pull_dist2d->Fill(mag(v[ilsp][0] - (vtx.x - x0),
                            v[ilsp][1] - (vtx.y - y0)) / vtx.bs2derr);
    ...
    if (!lsp_matched[ilsp]) {
      ...
      for (int j = 0; j < ndau; ++j) {
        const reco::GenParticle* p1 = partons[ilsp][j];
        if (is_lepton(p1) || is_neutrino(p1) || p1->pt() < 20 || fabs(p1->eta()) > 2.5 || fabs(dbv[ilsp] * sin(p1->phi() - atan2(v[ilsp][1], v[ilsp][0]))) < 0.01) continue;
        parton_pt.push_back(p1->pt());
        sumpt += p1->pt();
      }
      std::sort(parton_pt.begin(), parton_pt.end(), [](float p1, float p2) { return p1 > p2; } );
    ...
    // histogram dBV
//    h_s_dbv->Fill(dbv[ilsp], mevent->bs2ddist(vtx));
//    if (dvv > 0.03) {
//      h_s_dbv_gendvv->Fill(dbv[ilsp], mevent->bs2ddist(vtx));
//    }

    h_s_dbv_betagamma->Fill(lsp_p4.Beta()*lsp_p4.Gamma(), dbv[ilsp]);
    h_s_drmax_betagamma->Fill(lsp_p4.Beta()*lsp_p4.Gamma(), vtx.drmax());
    h_s_drmax_dbv->Fill(dbv[ilsp], vtx.drmax());
    h_s_gendrmax_dbv->Fill(dbv[ilsp], drmax);

    h_s_dx_drmax->Fill(vtx.drmax(), (vtx.x - x0) - v[ilsp][0]);
    h_s_dy_drmax->Fill(vtx.drmax(), (vtx.y - y0) - v[ilsp][1]);
    h_s_dz_drmax->Fill(vtx.drmax(), (vtx.z - z0) - v[ilsp][2]);
    h_s_dist2d_drmax->Fill(vtx.drmax(), mag(v[ilsp][0] - (vtx.x - x0), v[ilsp][1] - (vtx.y - y0)));
    h_s_dist3d_drmax->Fill(vtx.drmax(), mag(v[ilsp][0] - (vtx.x - x0), v[ilsp][1] - (vtx.y - y0), v[ilsp][2] - (vtx.z - z0)));
  ...
  for (int ilsp = 0; ilsp < 2; ++ilsp) {
    bool matched = false;
    for (const MFVVertexAux& vtx : *vertices) {
      double dist = mag(v[ilsp][0] - (vtx.x - x0),
                        v[ilsp][1] - (vtx.y - y0),
                        v[ilsp][2] - (vtx.z - z0));
      if (dist < max_dist) {
        matched = true;
    ...
    if (matched) {
      h_gen_dbv_matched->Fill(dbv[ilsp]);
    }
  }
  ...
  if (nsv >= 2) {
    ...
    // histogram dVV
    h_s_dvv->Fill(dvv, mag(v0.x - v1.x, v0.y - v1.y));
  }
  ...
  for (int ilsp = 0; ilsp < 2; ++ilsp) {
    for (int ivtx = 0; ivtx < int(vertices->size()); ++ivtx) {
       double dist = mag(v[ilsp][0] - (vertices->at(ivtx).x - x0),
                         v[ilsp][1] - (vertices->at(ivtx).y - y0),
                         v[ilsp][2] - (vertices->at(ivtx).z - z0));
      if (dist < max_dist) {
        vtxmatch[ilsp] = ivtx;
        break;
      }
  ...
  if (vtxmatch[0] >= 0 && vtxmatch[1] >= 0) {
    h_gen_dvv_matched->Fill(dvv);
    if (dvv > 0.06) {
      h_gen_dvv_matched_600um->Fill(dvv);
    }
  }
