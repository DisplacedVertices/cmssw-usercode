...
//      float densityFactor = 2./sqrt(20.*tracks.size()); // assuming all tracks being in 2 narrow jets of cone 0.3
      ...
      for(std::vector<reco::TransientTrack>::const_iterator tt = tracks.begin();tt!=tracks.end(); ++tt )   {
       ...
       if(dist.calculate(tt->impactPointState(),seed.impactPointState()))
		 ...
		 if (do_histos) h_tcfds_mval->Fill(m.value());
		 if (do_histos) h_tcfds_msig->Fill(m.significance());

                 float distanceFromPV =  (dist.points().second-pv).mag();
                 float distance = dist.distance();
		 if (do_histos) h_tcfds_distfrompv->Fill(distanceFromPV);
		 if (do_histos) h_tcfds_dist->Fill(distance);
		 ...
		 if (do_histos) h_tcfds_dotprodTrack->Fill(dotprodTrack);
		 if (do_histos) h_tcfds_dotprodSeed ->Fill(dotprodSeed);

                 float w = distanceFromPV*distanceFromPV/(pvDistance*distance);
		 if (do_histos) h_tcfds_w->Fill(w);
...
//                    dotprodTrackSeed2D > clusterMinAngleCosine && //Angle between track and seed
        //      distance*clusterScale*tracks.size() < (distanceFromPV+pvDistance)*(distanceFromPV+pvDistance)/pvDistance && // cut scaling with track density
		 ...
		 if (do_histos) h_tcfds_sel->Fill(selected);
		 if (do_histos) h_tcfds_disttimesscale->Fill(distance*clusterScale);
		 if (do_histos) h_tcfds_densitytimesdist->Fill(densityFactor*distanceFromPV);
      ...
      if (do_histos) h_tcfds_sumw->Fill(sumWeights);
 ...
 )
{
	...
	for(std::vector<TransientTrack>::const_iterator it = selectedTracks.begin(); it != selectedTracks.end(); it++){
		...
		if (do_histos) h_tcfds_trackip3dsuccess->Fill(ip.first);
		if (do_histos) h_tcfds_trackip3d->Fill(ip.second.value());
		if (do_histos) h_tcfds_trackip3dsig->Fill(ip.second.significance());
...
#endif
                    seeds.push_back(*it);  
		    if (do_histos) h_tcfds_seedtrackip3d->Fill(ip.second.value());
		    if (do_histos) h_tcfds_seedtrackip3dsig->Fill(ip.second.significance());
                  }
