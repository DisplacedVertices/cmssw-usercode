...
#sample_files(process, 'qcdht1500_2016APV', 'TrackingTreerUL16APV', 1)
...
if True:
    process.load('JMTucker.Tools.RescaledTracks_cfi') #Alec changed
...
#    setattr(process, 'p%s' % name, cms.Path(common * filt * tk))     #Alec commented to get rid of Heavy and Light files
...
#        samples = pick_samples(dataset, qcd=True, ttbar=False, all_signal=False, data=False, bjet=False, span_signal=True) # no data currently; no sliced ttbar since inclusive is used
    ...
    #set_splitting(samples, 'TrackingTreerUL16APV', 'default', json_path('ana_2016.json'), 16)
